...
    and angle value (in degree).
    ...
    if indicator == "h":  # boundary cases check
        ...
        # logging.debug("BO: %s" % BC_left)

        # calculate up left (90-180)
        if  cntr_coord[1] - AB_left > 0:
            x0 = 0
        ...
        # logging.debug("B'C': %s" % BC_right)

        # calculate up right
        if AB_right < y_lim - cntr_coord[1]:
            x1 = x_lim
...
start, end = lineSlice(img, angle, cntr)
